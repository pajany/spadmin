...
export class ProductsComponent
  ...
  filterForm() {
    ...
    this.subscriptions.push(
      this.filterGroup.controls.lot_active.valueChanges.subscribe(() =>
        this.filter()
      )
    );
    this.subscriptions.push(
  ...
  filter() {
    const filter = {};
    const status = this.filterGroup.get('lot_active').value;
    if (status) {
      filter['lot_active'] = status;
    }
  ...
  */
  ...
  download(data){
    ...
    let jsondata =JSON.parse(JSON.stringify(data.source.value));
     this.downloadFile(jsondata, 'Manage Customer');
   }
 
   downloadFile(data, filename='data') {
 
     let csvData = this.ConvertToCSV(data, ['lot_no','located','client_name','email','address','phone','state','city']);
     console.log(csvData)
